...
double *      stateFreq; /* Counts of state frequencies */
 ...
 *     policy[numStates] 
 *     utilities[numStates]
 *     stateFreq[numStates], all entries initialized to zero
 *
 * Progenitor
 *   Jerod Weinman
 */
...
td_initialize ( mdp * p_mdp_in, double gamma_in )
{
  ...
  stateFreq = calloc ( p_mdp->numStates, sizeof(double) );

  if (NULL == stateFreq)
  {
    fprintf (stderr, "td_initialize: Unable to allocate stateFreq (%s)",
             strerror (errno));
    exit (EXIT_FAILURE);
  }
...
{
  if (stateFreq[state] == 0) {
	utilities[state] = reward;
  } if (prevValid == true) {
	stateFreq[prevState]++;
	utilities[prevState] = utilities[prevState] + 
	  updateWeight(stateFreq[prevState]) 
	  * 
	  (prevReward + gamma * utilities[state] - utilities[prevState]);
